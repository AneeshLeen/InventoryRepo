...
{
    public partial class fReturns : Form
    {
      ...
      void  RefReshList()
        {
             try
            {
                ...
                {
                    var _Returns = db.Returns.Where(sr => sr.CustomerID > 0).OrderByDescending(so => so.InvoiceNo);
                    ...
                    if (_ReturnList != null)
                    {
                        ...
                        foreach (Return oReturn in _ReturnList)
                        {
                            ...
                            if (oCustomers != null)
                            {
                                Customer oCustomer = oCustomers.FirstOrDefault(o => o.CustomerID == oReturn.CustomerID);
                                dr["Customer"] = oCustomer != null ? oCustomer.Name : "";
                                dr["ContactNo"] = oCustomer != null ? oCustomer.ContactNo : "";
                                dr["DueAmount"] = oCustomer != null ? oCustomer.TotalDue : 0;
        ...
        private void fReturns_Load(object sender, EventArgs e)
        {
            RefReshList();
            ControlPermission();
        }
