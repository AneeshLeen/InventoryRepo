...
namespace INVENTORY.UI
{
    public partial class fYearlySaleReport : Form
    {
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            if(chkSummary.Checked)
            {
                using (DEWSRMEntities db1 = new DEWSRMEntities())
                {
                    using (var connection = db1.Database.Connection)
                    {
                        ...
                        string embededResource = "INVENTORY.UI.RDLC.rptYearlySalesOrderSummary.rdlc";
            ...
            else
            {
                using (DEWSRMEntities db1 = new DEWSRMEntities())
                {
                    using (var connection = db1.Database.Connection)
                    {
                        ...
                        string embededResource = "INVENTORY.UI.RDLC.rptYearlySalesOrder.rdlc";
                        ReportParameter rParam = new ReportParameter();
                        List<ReportParameter> parameters = new List<ReportParameter>();
                        rParam = new ReportParameter("Month", "Salse For the Year: " + cboYear.Text);
                        parameters.Add(rParam);
            ...
            //        ds.Tables.Add(dt);
            //        string embededResource = "INVENTORY.UI.RDLC.rptMonthlyOrder.rdlc";
            //        ReportParameter rParam = new ReportParameter();
            //        List<ReportParameter> parameters = new List<ReportParameter>();
            //        rParam = new ReportParameter("Month", "Salse For the Year: " + cboYear.Text);
            //        parameters.Add(rParam);
