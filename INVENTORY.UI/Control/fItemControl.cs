...
{
    public partial class fItemControl : Form
    {
        ...
        private void RefreshList()
        {
            try
            {
                using (DEWSRMEntities db = new DEWSRMEntities())
                {
                    ...
                    else if (_ObjectName == "Godown")
                    {
                        this.Text = "Ware house Control";
                        var _Godons = db.Godowns;

                        if (_Godons != null)
                        {
                            foreach (Godown grd in _Godons)
                            {
                                dr = dt.NewRow();
                                dr["ID"] = grd.GodownID;
                            ...
                            grdItems.DataSource = dt;
                            lblTotal.Text = "Total :" + _Godons.Count().ToString();
                        }
                    }
                    ...
                    else if(_ObjectName=="Bank")
                    {
                        ...
                        if (_BankInfos != null)
                        {
                            foreach (Bank grd in _BankInfos)
                            {
                                ...
                                dr["ID"] = grd.BankID;
                                dr["Code"] = grd.Code;
                                dr["Name"] = grd.BankName+", "+grd.BranchName;
        ...
        private void grdItems_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (nID > 0)
                {
                    ...
                    {
                        this.Text = "Warehouse Control";
                        oGodown = db.Godowns.FirstOrDefault(p => p.GodownID == nID);
                        _ctl.SelectedID = oGodown.GodownID;
                        _ctl.Code = oGodown.GodownCode;
                        _ctl.Name = oGodown.GodownName;
                    }
                    else if(_ObjectName=="Bank")
                    {
                        this.Text = "Bank Control";
